...
#
#####################################################################
import time,sys,_thread,os,subprocess
...
if not os.path.isfile("/var/artillery/artillery.py"):
    ...
    if not sys.stdin.isatty():
        setup_command.append("--non-interactive")
        if os.environ.get("ARTILLERY_ASSUME_YES", "").lower() in ("1", "true", "yes", "y"):
            setup_command.append("--assume-yes")
    setup_result = subprocess.run(setup_command, check=False)
    if setup_result.returncode != 0:
        print("[!] setup.py exited with a non-zero status: %s" % setup_result.returncode)
    sys.exit()
...
try:
    ...
    if is_posix():
        time.sleep(2)
        _thread.start_new_thread(create_iptables_subset, ())
    ...
    if is_config_enabled("SOURCE_FEEDS"):
        _thread.start_new_thread(pull_source_feeds, ())
